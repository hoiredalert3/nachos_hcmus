...
public:
	...
	{
		return fileDescriptorTable->Insert(name, openMode);
	}

	int Close(int id)
	{
		return fileDescriptorTable->Remove(id);
	}

	int Read(char *buffer, int charCount, int id)
	{
		return fileDescriptorTable->Read(buffer, charCount, id);
	}

	int Write(char *buffer, int charCount, int id)
	{
		return fileDescriptorTable->Write(buffer, charCount, id);
	}

	int Seek(int position, int id)
	{
		return fileDescriptorTable->Seek(position, id);
	}
